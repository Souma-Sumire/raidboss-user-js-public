if (
  ...
  const check = (effects, triggerSetConfig, channel = "e") => {
    ...
    const result = effects
      .map((e) => {
        if (!e.Effects) {
          return triggerSetConfig.hunger_police_noFood
            .replace("${name}", e.Name)
            .replace("${time}");
        }
        if (e.Effects.Timer < time * 60) {
          return triggerSetConfig.hunger_police_hasFood
            .replace("${name}", e.Name)
            .replace("${time}", Math.floor(e.Effects.Timer / 60));
        }
      })
      .filter(Boolean);
    result.map((v) => {
    ...
    doTextCommand(
      result.length
        ? `/${channel.trim()} ${add}`
        : `/e ${triggerSetConfig.hunger_police_all_full}`
    );
    return result.join(",");
  };
  ...
  Options.Triggers.push({
    ...
    config: [
      ...
      {
        id: "hunger_police_time",
        name: { en: "报警阈值（分钟）" },
        type: "string",
        default: "20",
      },
      {
        id: "hunger_police_hasFood",
        name: { en: "文本：还有" },
        type: "string",
        default: "${name}食物仅剩${time}分钟",
      },
      {
        id: "hunger_police_noFood",
        name: { en: "文本：没吃" },
        type: "string",
        default: "${name}没吃食物",
      },
      {
        id: "hunger_police_all_full",
        name: { en: "文本：都吃饱了" },
        type: "string",
        default: "都吃饱了",
      },
      {
        id: "hunger_police_add",
        name: { en: "报警音" },
        type: "string",
        default: "<se.1>",
      },
    ...
    triggers: [
      {
        ...
        promise: async (data, matches) => {
          ...
          const ids = data.party.partyIds_.length
            ? data.party.partyIds_.map((v) => parseInt(v, 16))
            : undefined;
          const effects = combatants
            .filter((v) => (ids ? ids.includes(v.ID) : v.Name === data.me))
            .map((v) => ({
              Name: data.ShortName(v.Name),
              Effects: v.Effects?.find((e) => e.BuffID === 48),
            }));
          data.hunger_police_text = check(
            effects,
