const hotbar = {
  standard: [2, 1], // 标准舞步放在热键栏的哪 默认第2热键栏的第1个栏位上
...
if (new URLSearchParams(location.search).get("alerts") !== "0" && /raidboss\.html/.test(location.href)) {
  ...
  const countdownReg = /^.{14} ChatLog 00:(?:00B9|0[12]39)::(?:距离战斗开始还有|Battle commencing in |戦闘開始まで)(?<cd>\d+)[^（]+（/i;
  ...
  function handlePlayerChangedEvent(e) {
    ...
    if (playerJob === "DNC") {
      ...
      if (needsSteps.length && curStep < needsSteps.length) {
        ...
        postNamazu(`/hotbar set ${step[lang][["Emboite", "Entrechat", "Jete", "Pirouette"].indexOf(needsSteps[curStep])]} ${hotbar[dance].join(" ")}`);
