...
const getRotationDirection = (p1, p2) => {
  ...
  if (cross > 0) return "counter-clockwise";
  ...
  triggers: [
    ...
    },
    {
      ...
      alertText: (data, matches, output) => {
        ...
        if (Directions.hdgTo8DirNum(parseFloat(matches.heading)) % 2 === 1) {
          ...
          const final = data.souma竹子预占位.filter((x) =>
            adjacentNumber.includes(x)
          );
          if (final.length === 0) {
            console.error(
              "Souma 十字光牙 预占位错误",
    ...
    // 1128风
    {
      ...
      response: (data, matches, output) => {
        ...
        return {
          infoText: output[attb]({ player: data.party.member(matches.target) }),
        };
      },
    },
    {
    ...
    },
    {
      ...
      infoText: (data, _matches, output) => {
        if (
          data.triggerSetConfig.四连刀时报分散分摊 === false &&
          data.souma四连中
        ) {
          return;
        }
    ...
    },
    {
      ...
      condition: (data, matches) => {
        ...
        if (
          data.soumaPhase === "P1-后大地1" ||
          data.soumaPhase === "P1-后大地2"
        ) {
          const targetRole =
            data.party.nameToRole_[matches.target] === "dps" ? "dps" : "th";
    ...
    },
    {
      ...
      promise: async (data) => {
        if (
          data.souma四连CloneIDs.length === 4 ||
          data.souma四连CloneIDs.length === 1
        ) {
          data.soumaCombatantData = (
            await callOverlayHandler({
              call: "getCombatants",
      ...
      response: (data, _matches, output) => {
        ...
        if (data.souma四连CloneIDs.length === 4) {
          const clones = data.soumaCombatantData.sort((a, b) => {
            return (
              data.souma四连CloneIDs.indexOf(a.ID) -
              data.souma四连CloneIDs.indexOf(b.ID)
            );
          });
          const res = [];
          clones.forEach((c) => {
    ...
    },
    {
      ...
      netRegex: { id: "0017" },
      condition: (data, matches) =>
        data.party.nameToRole_[matches.target] === "tank",
      response: (data, matches, output) => {
        // cactbot-builtin-response
        output.responseOutputStrings = {
          tankBuster: { en: "风圈点名" },
    ...
    },
    {
      ...
      response: (data, matches, output) => {
        ...
        const targetIndex = data.souma风狼阵.find(
          (v) => v.name === matches.target
        ).index;
        const myIndex = data.souma风狼阵.find((v) => v.name === data.me).index;
    ...
    },
    {
      ...
      promise: async (data) => {
        data.soumaCombatantData = (
          ...
          data.party.details.find((p) => p.name === v.Name)
        );
      },
      alertText: (data, matches, output) => {
        const handler = (f) =>
        ...
        const playersFloor = data.soumaCombatantData.map((v) =>
          getFloor(v.PosX, v.PosY)
        );
        // 以玩家数量最多的台子作为起始点
    ...
    },
    {
      ...
      condition: (data, matches) =>
        data.me === matches.target && data.souma闪光炮.length > 0,
      delaySeconds: 0.5,
      promise: async (data) => {
        data.soumaCombatantData = (
          await callOverlayHandler({
    ...
    },
    {
      ...
      infoText: (data, _matches, output) =>
        output[
          `${data.role}-${data.souma远近线.line}-w/${data.souma远近线.partnerRole}`
        ]({
    ...
    },
    {
      ...
      alertText: (data, _matches, output) => {
        if (data.souma不堪一击.length === 2) {
          ...
          // A461 左安全
          // A461 左安全
          const heading = data.souma不堪一击.filter(
            (v) => v.id === "A45F" || v.id === "A461"
          )[0].heading;
          const headingFloor =
            HdgDirToFloor[
              Directions.hdgTo16DirNum(parseFloat(heading)).toString()
            ];
          if (data.soumaCombatantData.length === 0) {
            return output[
              `${data.souma不堪一击
          ...
          const sub =
            Math.abs(rawSub) >= 3 && Math.abs(rawSub) <= 5
              ? rawSub < 0
                ? rawSub + 5
                : rawSub - 5
              : rawSub;
    ...
    // 暂时不知为何2种ID 似乎AA02是第一轮 之后都是A494
    {
      ...
      alertText: (data, _matches, output) =>
        output.text({ count: data.souma八连光弹Counter }),
