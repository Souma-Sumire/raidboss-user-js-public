...
Options.Triggers.push({
  ...
  triggers: [
          ...
          if (data?.bloodrakeTheSecondTimeIsYourself) {
            if (data?.bloodrakeIsYourself === 1) {
              if (data.postNamazuTextCommand) {
                doTextCommand(
                  `/p TN：${data.role !== "dps" ? output.bloodrakeIsYourself01() : output.bloodrakeIsYourself10()}`,
                );
                doTextCommand(
                  `/p DPS：${data.role !== "dps" ? output.bloodrakeIsYourself10() : output.bloodrakeIsYourself01()}`,
                );
              }
            ...
            if (data?.bloodrakeIsYourself === 2) {
              if (data.postNamazuTextCommand) {
                doTextCommand(
                  `/p TN：${data.role !== "dps" ? output.bloodrakeIsYourself11() : output.bloodrakeIsYourself00()}`,
                );
                doTextCommand(
                  `/p DPS：${data.role !== "dps" ? output.bloodrakeIsYourself00() : output.bloodrakeIsYourself11()}`,
                );
              }
            ...
            if (data?.bloodrakeIsYourself === 0) {
              if (data.postNamazuTextCommand) {
                doTextCommand(
                  `/p TN：${data.role !== "dps" ? output.bloodrakeIsYourself00() : output.bloodrakeIsYourself11()}`,
                );
                doTextCommand(
                  `/p DPS：${data.role !== "dps" ? output.bloodrakeIsYourself11() : output.bloodrakeIsYourself00()}`,
                );
              }
            ...
            if (data?.bloodrakeIsYourself === 1) {
              if (data.postNamazuTextCommand) {
                doTextCommand(
                  `/p TN：${data.role !== "dps" ? output.bloodrakeIsYourself10() : output.bloodrakeIsYourself01()}`,
                );
                doTextCommand(
                  `/p DPS：${data.role !== "dps" ? output.bloodrakeIsYourself01() : output.bloodrakeIsYourself10()}`,
                );
              }
...

    {
      ...
      response: (data, matches, output) => {
        ...
        if (id === "012C")
          return { infoText: output.blueTetherDir({ dir: dirStr }), tts: output.blueTetherDirTTS({ dir: dirStr }) };
        if (id === "012D")
          return {
    ...
    },
    {
      ...
      alertText: (data, _matches, output) => {
        ...
        return data.me === result
          ? output.pullLineNowYou()
          : output.pullLineNowAnother({ name: data.ShortName(result) });
      },
    ...
    },
    {
      ...
      condition: (data, matches) =>
        data.act === "curtain" && data.me === matches.target && Math.ceil((parseFloat(matches.duration) - 2) / 10) > 1,
      delaySeconds: 2,
      infoText: (_data, matches, output) =>
        output.act6Group({ group: Math.ceil((parseFloat(matches.duration) - 2) / 10) }),
      outputStrings: {
        act6Group: {
          en: "You are No.${group} ",
          cn: "你是第${group}组",
    ...
    },
    {
      ...
      delaySeconds: (data, matches) =>
        parseFloat(matches.duration) - 7 - (data.party.nameToRole_[matches.target] === "dps" ? 5 : 0),
      alarmText: (data, matches, output) => {
        return data.me === matches.target
          ? output.pullLineNowYou()
          : output.pullLineNowAnother({ name: data.ShortName(matches.target) });
      },
    ...
    },
    {
      ...
      netRegex: NetRegexes.gainsEffect({ effectId: "AF4", capture: true }),
      condition: (data, matches) =>
        data.act === "curtain" && !(matches.duration < 13 && data.party.nameToRole_[matches.target] === "dps"),
      delaySeconds: (data, matches) =>
        parseFloat(matches.duration) - 7 - (data.party.nameToRole_[matches.target] === "dps" ? 5 : 0) - 3,
  ...
  timeline: [
    'hideall "Inversive Chlamys"',
    'hideall "--role debuffs--"',
    'hideall "--element debuffs--"',
    'hideall "Elemental Belone"',
  ],
  timelineReplace: [
    {
