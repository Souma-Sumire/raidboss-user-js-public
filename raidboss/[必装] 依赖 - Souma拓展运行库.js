...
function mark(
  ...
  if (isNotInRaidboss) {
    console.debug("邮差mark", actorHexID, markType);
  } else {
    callOverlayHandler({
      call: "PostNamazu",
      c: "mark",
...
function clearMark(localOnly = false) {
  if (localOnly) {
    doQueueActions(
      markTypes.map((v) => ({
        c: "mark",
        p: JSON.stringify({ ActorID: 0xe000000, MarkType: v, LocalOnly: true }),
      })),
...
function defaultSort() {
  ...
  const role = {
    ...
    dps: [
      2, 4, 5, 7, 20, 22, 23, 25, 26, 27, 29, 30, 31, 34, 35, 36, 38, 39, 41,
      42,
    ],
  };
  let t = 0;
